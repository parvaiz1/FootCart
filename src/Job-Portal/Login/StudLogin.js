...
function StudentLogin(props) {
  ...
  const login = useGoogleLogin({
    onSuccess: async (response) => {
      try {
        ...
        await axios.post("http://localhost:8080/StudentProfile/Glogin", { userId, ipAddress, email, name, gtoken, isApproved })
          .then((response) => {
  ...
  async function sendOtp() {
    await axios.post("http://localhost:8080/StudentProfile/otpSignUp", { PhoneNumber })
      .then((res) => {
  ...
  async function confirmOtp() {
    ...
    await axios.post("http://localhost:8080/StudentProfile/verifyOtp", {ipAddress, otp , isApproved})
      .then((res) => {
        // console.log(res.data)
        let result = res.data
            let token = result.token
